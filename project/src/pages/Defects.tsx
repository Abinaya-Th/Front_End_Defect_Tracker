...
export const Defects: React.FC = () => {
  ...
  const { defects, projects, releases, addDefect, updateDefect, deleteDefect, setSelectedProjectId } = useApp();
  ...
  const [isRejectionCommentModalOpen, setIsRejectionCommentModalOpen] = useState(false);
  ...
  const filteredDefects = defects.filter(d =>
    ...
    (filters.id === '' || d.id.toLowerCase().includes(filters.id.toLowerCase())) &&
    ...
    (filters.assignedTo === '' || (d.assignedTo || '').toLowerCase().includes(filters.assignedTo.toLowerCase())) &&
    (filters.search === '' ||
      ...
      (d.subModule || '').toLowerCase().includes(filters.search.toLowerCase()) ||
      (d.type || '').toLowerCase().includes(filters.search.toLowerCase()) ||
      (d.severity || '').toLowerCase().includes(filters.search.toLowerCase()) ||
      (d.priority || '').toLowerCase().includes(filters.search.toLowerCase()) ||
      (d.status || '').toLowerCase().includes(filters.search.toLowerCase()) ||
      (d.assignedTo || '').toLowerCase().includes(filters.search.toLowerCase())
    )
  );
  ...
  const modulesList = projectId && mockModules[projectId] ? mockModules[projectId].map(m => m.name) : [];
  const submodulesList = formData.module && projectId && mockModules[projectId]
    ? (mockModules[projectId].find(m => m.name === formData.module)?.submodules || [])
    : [];

  // Unique values for dropdowns
  const uniqueModules = modulesList;
  const uniqueSubmodules = formData.module && modulesList.includes(formData.module)
    ? submodulesList
    : Array.from(new Set(defects.filter(d => d.projectId === projectId).map(d => d.subModule).filter(Boolean)));
  const uniqueTypes = Array.from(new Set(defects.filter(d => d.projectId === projectId).map(d => d.type).filter(Boolean)));
  const uniqueSeverities = Array.from(new Set(defects.filter(d => d.projectId === projectId).map(d => d.severity).filter(Boolean)));
  const uniquePriorities = Array.from(new Set(defects.filter(d => d.projectId === projectId).map(d => d.priority).filter(Boolean)));
  const uniqueStatuses = Array.from(new Set(defects.filter(d => d.projectId === projectId).map(d => d.status).filter(Boolean)));
  const uniqueAssignedTo = Array.from(new Set(defects.filter(d => d.projectId === projectId).map(d => d.assignedTo).filter(Boolean)));
        ...
        <div className="grid grid-cols-8 gap-4 items-end">
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setFilters(f => ({ ...f, module: e.target.value, subModule: '' }))}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Submodule</label>
            <select
            ...
            <div className="flex items-center ml-auto">
              ...
              />
                ...
                <div className="overflow-x-auto">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Submodule</th>
                ...
                {filteredDefects.length > 0 ? filteredDefects.map(defect => (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{defect.type}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{defect.assignedTo}</td>
                        ...
                        <button onClick={() => handleEdit(defect)} className="p-1 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded" title="Edit Defect">
                                  <Edit2 className="w-4 h-4" />
                                </button>
                        <button onClick={() => handleDelete(defect.id)} className="p-1 text-red-600 hover:text-red-800 hover:bg-red-50 rounded" title="Delete Defect">
                  ...
                  <tr>
                    <td colSpan={10} className="p-12 text-center text-gray-500">
                      ...
                      <div className="text-lg font-medium text-gray-900 mb-2">No defects found</div>
                      <div className="text-gray-500 mb-4">No defects have been reported for this project</div>
                      <Button onClick={() => setIsModalOpen(true)} icon={Plus}>
                        Add Defect
                      </Button>
                    </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button type="submit">{editingDefect ? 'Update Defect' : 'Report Defect'}</Button>
          </div>
        </form>
      </Modal>
